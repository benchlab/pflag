...
// Related to https://github.com/benchlab/goguidance/issues/521.
...
func TestChangingArgs(t *testing.T) {
	...
	os.Args = []string{"guidance", "--before", "subguidance"}
	before := Bool("before", false, "")
	if err := GetCommandLine().Parse(os.Args[1:]); err != nil {
		t.Fatal(err)
	}
	guidance := Arg(0)
	os.Args = []string{"subguidance", "--after", "args"}
	after := Bool("after", false, "")
	Parse()
	args := Args()

	if !*before || guidance != "subguidance" || !*after || len(args) != 1 || args[0] != "args" {
		t.Fatalf("expected true subguidance true [args] got %v %v %v %v", *before, guidance, *after, args)
	}
}
